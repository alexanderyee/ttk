...
func set_total_health(health:  float):
	total_health = health
	size.x = max_width

func update_health_bar(curr_health: float, dmg_taken: float):
	curr_health_bar.custom_minimum_size.x = curr_health / total_health * health_bar_container.size.x
	dmg_taken_bar.custom_minimum_size.x = dmg_taken / total_health * health_bar_container.size.x
